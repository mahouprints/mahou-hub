...
- **Compra de estocável NÃO vira `Custo`.** Recibo com filamento/insumo gera só movimento de estoque `COMPRA`; `Custo` é criado apenas para item `NAO_ESTOCAVEL` (frete, ferramenta, manutenção). O motivo é o próprio dashboard: ele desconta do lucro tanto o `Custo` do mês quanto o filamento consumido em cada venda — lançar a compra do rolo faria o mesmo dinheiro sair duas vezes do resultado.
- **A leitura de nota nunca grava direto.** `POST /recibos/:id/extrair` chama o Gemini e deixa o recibo em `EXTRAIDO`; estoque e custos só se movem no `POST /recibos/:id/confirmar`, depois da revisão humana. Campo que a IA não leu volta `null` e entra em `camposIlegiveis` — a UI pede outra foto em vez de completar o buraco. O prompt proíbe explicitamente calcular: multiplicação de unitário × quantidade é feita no nosso código (`completarValores`), onde o resultado é sempre o mesmo.
- **Casamento de item da nota com o cadastro é conservador de propósito** (`casarComCadastro`): exige que todas as palavras do nome cadastrado apareçam na descrição da nota, e devolve null em empate. Casar errado move saldo do rolo errado sem ninguém ver; não casar só faz a tela perguntar.
...
- **`ModeloMakerWorld` é separado de `ProdutoOportunidade` de propósito.** Oportunidade é um anúncio de marketplace (tem preço praticado e vendas observadas); modelo MakerWorld é um ARQUIVO (tem licença, peso e tempo de impressão, não tem preço nem vendas). Juntar os dois deixaria metade dos campos null nos dois sentidos. Ambos viram `Produto` pelo mesmo padrão (`produtoId` unique nullable).
- **Licença é o filtro nº 1 da prospecção MakerWorld — 92,1% do acervo é invendável** (varredura de 56.873 modelos em 2026-07-27). `Standard Digital File License` (a padrão do site) proíbe expressamente cobrar; as CC `-NC` idem; `MakerWorld Exclusive` idem. Só `CC0`, `BY`, `BY-SA` e `BY-ND` passam. O bot descarta antes de gravar — não existe registro invendável no banco. **Licença desconhecida é tratada como proibida**, nunca como permitida.
- **`licencaObrigacao` não é decorativo**: `BY`/`BY-SA`/`BY-ND` exigem creditar o autor na descrição do anúncio, e `BY-ND` proíbe vender versão modificada. A UI mostra esse texto em todo modelo que não seja `CC0`.
- **Métrica de priorização é `lucroPorHoraCentavos`, não margem %.** Com uma A1 só, o gargalo é hora de máquina: peça de 30% de margem em 1h ganha de uma de 50% em 8h. A tela ordena por nota da IA, mas o score objetivo pondera lucro/hora.
- **Peso e tempo saem do perfil mais barato publicado pelo autor** (sem AMS na frente, depois menor gramatura). Um modelo com versão "AMS" de 231g e "sem AMS" de 84g é um modelo de 84g pra efeito de custo.
